...
class Pantalla6View:
    ...
    async def guardar_datos(self, e):
        self.actualizar()
        try:
            conn = get_connection()
            cursor = conn.cursor()
            ...
            await self.page.shared_preferences.set("cinta_volumen", self.volumen_input.value)
            await self.page.shared_preferences.set("unidad_superficie", self.superficie_input.value)
            await self.page.shared_preferences.set("documento_cliente", self.documento_cliente)
            await self.page.shared_preferences.set("pliegos", self.pliegos_input.value)
    ...
    def armar_vista(self):
        logo = ft.Image(src=self.resource_path("imagen/Printers.png"), width=150, height=75, fit=ft.BoxFit.CONTAIN)
        ...
        header = ft.Row(
            controls=[
                ...
                ft.Icon(icon=ft.Icons.ACCOUNT_CIRCLE, size=40, color="black")
            ],
        ...
        self.contenedor_pagina.content = ft.Column([
            ...
            ft.Divider(color="white"),
            impresion,
            ft.Container(content=botones, padding=ft.Padding(top=120))
        ])

    def texto_bloque(self, texto, padding_left=14):
        return ft.Container(
            content=ft.Text(value=texto, color="white", size=16),
            padding=ft.Padding(left=padding_left, top=10),
            width=180,
            height=40,
            bgcolor="#1d4fe0",
