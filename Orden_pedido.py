...
class OrdenPedidoView:
    def __init__(self, page: ft.Page, documento_cliente):
        ...
        self.mes_recepcion = ft.Dropdown(label="Mes", width=130, on_select=self.actualizar_dias_recepcion)
        self.anio_recepcion = ft.Dropdown(label="Año", width=120)

        self.dia_entrega = ft.Dropdown(label="Día", width=100)
        self.mes_entrega = ft.Dropdown(label="Mes", width=130, on_select=self.actualizar_dias_entrega)
        ...
        self.cargar_datos()
        self.anio_entrega.on_select = self.actualizar_dias_entrega
        self.mes_entrega.on_select = self.actualizar_dias_entrega

        self.anio_recepcion.on_select = self.actualizar_dias_recepcion
        self.mes_recepcion.on_select = self.actualizar_dias_recepcion
    ...
    def texto_bloque(self, texto, padding_left=60, padding_top=15, margin=None):
        return ft.Container(
            ...
            margin=margin if margin else ft.Margin(left=20),
            padding=ft.Padding(left=padding_left, top=padding_top),
            width=240,
            height=50,
            bgcolor="#1d4fe0",
    ...
    def view(self):
        ...
        Datos_columnas = ft.Column([
            contenedor,
            ft.Container(content=self.cliente_field, width=340, margin=ft.Margin(left=30)),
            ft.Container(content=self.publicidad_field, width=340, margin=ft.Margin(left=30)),
            ft.Container(content=self.trabajo_field, width=340, margin=ft.Margin(left=30))
        ], spacing=30)
        ...
        lista2 = ft.Column([
            ...
            *[ft.Container(detalle, margin=ft.Margin(left=120)) for detalle in self.detalles]
        ], spacing=10)

        lista4 = ft.Row([
            self.texto_bloque("Material", padding_left=60),
            self.material_dropdown,
            self.texto_bloque("Cant. Material", padding_left=40, margin=ft.Margin(left=400)),
        ...
        lista5 = ft.Row([
            ...
            self.texto_bloque("Espesor", padding_left=70, margin=ft.Margin(left=350)),
